...
(() => {
  ...
  async function send(form,api,payload,onSuccess){if(busy)return;busy=true;revision++;const controls=[...form.querySelectorAll('input,select,textarea,button')];controls.forEach(e=>e.disabled=true);const button=form.querySelector('button'),status=$('sendStatus');button.textContent='送出中…';status.textContent='傳輸中... 請稍候';try{const result=await CP.call(api,{participantId:profile.id,participantName:profile.name,epoch,...payload},{onProgress:message=>status.textContent=message});onSuccess(result);}catch(e){status.textContent=e.message;button.textContent='重新送出／確認結果';controls.forEach(x=>x.disabled=false);}finally{busy=false;}}
